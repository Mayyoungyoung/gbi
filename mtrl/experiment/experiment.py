...
class Experiment(checkpointable.Checkpointable):
    def __init__(self, config: ConfigType, experiment_id: str = "0"):
        ...
        self.action_space.seed(config.setup.seed)
        ...
        self.replay_buffer = hydra.utils.instantiate(
            ...
            task_num=self.config.env.num_envs,
    ...
    def startup_logs(self) -> None:
        ...
        with open(config_file, "w") as f:
            f.write(json.dumps(config_utils.to_dict(self.config), indent=4))
...
def prepare_config(config: ConfigType, env_metadata: EnvMetaDataType) -> ConfigType:
    ...
    if "guide_encoder" in config.agent:
        if key in config.agent.guide_encoder:
            guide_encoder_type_to_select = config.agent.guide_encoder[key]
        else:
            guide_encoder_type_to_select = config.agent.guide_encoder.type
        if guide_encoder_type_to_select in ["identity"]:
            # if the encoder is an identity encoder infer the shape of the input dim.
            config.agent.guide_encoder_feature_dim = env_metadata["env_obs_space"].shape[0]
